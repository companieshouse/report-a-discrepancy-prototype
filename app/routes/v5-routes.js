...
router.post('/v5/confirm-company', function (req, res) {
  res.redirect('/v5/psc-name')
})


// ******* discrepancy-details validation ********************************
router.get('/v5/discrepancy-details', function (req, res) {
  // Set URl
  res.render('v5/discrepancy-details', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/discrepancy-details', function (req, res) {
  var errors = []
...
router.get('/v5/check-your-answers', function (req, res) {
  // Set URl
...
router.get('/v5/obliged-entity-details-name', function (req, res) {
  // Set URl
  res.render('v5/obliged-entity-details-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/obliged-entity-details-name', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (req.session.data['fullName'] === '') {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/obliged-entity-email')
  }
})


// ******* psc-discrepancy-options validation ********************************
router.get('/v5/psc-discrepancy-options', function (req, res) {
  // Set URl
  res.render('v5/psc-discrepancy-options', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/psc-discrepancy-options', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['pscOptions'] === 'undefined') {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/discrepancy-details')
  }
})


// ******* discrepancy-category validation ********************************
router.get('/v5/discrepancy-category', function (req, res) {
  // Set URl
  res.render('v5/discrepancy-category', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/discrepancy-category', function (req, res) {
  // Create empty array
  var errors = [];
  ...
  if (typeof req.session.data['discrepancyCategory'] === 'undefined') {
    ...
    res.render('v5/discrepancy-category', {
      ...
      // User inputted value so move to next page
      res.redirect('/v5/discrepancy-type')
    }
})


// ******* psc-name validation ********************************
router.get('/v5/psc-name', function (req, res) {
  // Set URl
  res.render('v5/psc-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/psc-name', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['pscName'] === 'undefined') {
    ...
    res.redirect('/v5/discrepancy-details')
      ...
      res.redirect('/v5/psc-discrepancy-options')
    }
})


// ******* discrepancy-type validation ********************************
router.get('/v5/discrepancy-type', function (req, res) {
  // Set URl
  res.render('v5/discrepancy-type', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/discrepancy-type', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['discrepancyType'] === 'undefined') {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/obliged-entity-type')
  }
})


// ******* oblighed-entity-details-organisation validation ********************************
router.get('/v5/obliged-entity-details-organisation', function (req, res) {
  // Set URl
  res.render('v5/obliged-entity-details-organisation', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/obliged-entity-details-organisation', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (req.session.data['organisationName'] === '') {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/obliged-entity-details-name')
  }
})


// ******* oblighed-entity-email validation ********************************
router.get('/v5/obliged-entity-email', function (req, res) {
  // Set URl
  res.render('v5/obliged-entity-email', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/obliged-entity-email', function (req, res) {
  // Create empty array
  var errors = []
...
router.get('/v5/obliged-entity-type', function (req, res) {
  // Set URl
  res.render('v5/obliged-entity-type', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/obliged-entity-type', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['obligedType'] === 'undefined') {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/obliged-entity-details-organisation')
  }
})
...
router.post('/v5/sign-in', function (req, res) {
  ...
  if (emailHasError || passwordHasError) {
    ...
    // User inputted value so move to next page
    res.redirect('/v5/discrepancy-category')
  }
})
